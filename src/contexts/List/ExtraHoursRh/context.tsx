...
import { ContextPropsExtraHoursRh, ExtraHoursRhProps, PendingProps, ProjectExtraHoursProps, StatusHours } from "./types";
...
export const Provider = ({ children }: { children: React.ReactNode }) => {
    ...
    const [extraHoursRh, setExtraHoursRh] = useState<
        PendingProps[]>([])
    ...
    async function fetchList() {
        ...
        const { data } = await api.get(
            routes.extraHoursRH.listPending,{params: {}}
        )
    ...
    async function fetchStatusHours() {
        const { data } = await api.get(routes.extraHoursRH.listStatusHours)
